...
The upgrade procedure consists in four steps of which the first two needs to be done only if the Kafka cluster already existing don't use KRaft and KafkaNodePools:

1. Migrate existing Kafka cluster to KafkaNodePools (Optional)
2. Migrate from ZooKeeper to KRaft (Optional)
3. Upgrade the operator.
4. Upgrade the Kafka cluster.


### 1. Migrate existing Kafka cluster to KafkaNodePools

If already existing Kafka clusters do not use KafkaNodePools then this step is necessary to proceed with the upgrade. First a `KafkaNodePool` resource needs to be created. The resource should be labeled with the `strimzi.io/cluster: <CLUSTER_NAME>` label where `<CLUSTER_NAME>` is the Kafka resource name. Set the replica count and storage configuration to match your current Kafka cluster, also set the roles to broker. Here is an example manifest:
...
Or if you are using Kustomize you can simply add the resource manifest file to the `resources` array. By applying this resource, you switch Kafka to using node pools.

> [!NOTE]
> There is no change or rolling update and resources are identical to how they were before.

Enable support for node pools in the Kafka resource using the `strimzi.io/node-pools: enabled` annotation. Here is an example:
...
For more information about migrating to `KafkaNodePools` read the specific [documentation](https://strimzi.io/docs/operators/0.45.0/full/deploying#proc-migrating-clusters-node-pools-str)


### 2. Migrate from ZooKeeper to KRaft

ZooKeeper has been deprecated since Kafka version >= `3.5` and is set to be deprecated with version `4`. The new protocol is KRaft. In this step we will migrate an existing cluster which uses `KafkaNodePools` (necessary pre-requisite) from ZooKeeper to KRaft.

> [!IMPORTANT]  
> Be sure to have enough space for the provisioning of PVCs during the migration.
...
Apply the changes and wait for the operator to start the migration. Then monitor the cluster and wait that it reaches the `KRaftPostMigration` state. Then there are two choices:
...
The operator will configure the KRaft controllers without the ZooKeeper connection details, with the migration flag disabled, and roll them. When both brokers and controllers are working in KRaft mode, the operator will delete all the resources related to ZooKeeper as well. At this point, the user can delete the .spec.zookeeper section from the Kafka custom resource because it is not needed anymore.
...
Apply the new operator, the new deployment will start with the v0.45.0 running. If there are Kafka clusters of an unsupported version the operator will print an `unsupported version` warning. This will be resolved with the next step.

### 4. Upgrade the Kafka cluster

Edit the Kafka resource of the target cluster and change the version to `3.9.0` or another supported version (`3.6.0`, `3.6.1`, `3.6.2`, `3.7.0`, `3.7.1`). Also change the config section and add the following parameters:
...
If everything works correctly the upgrade can be finalized updating then the Kafka resource to use the new `metadataVersion` version, for example:
